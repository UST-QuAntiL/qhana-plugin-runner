...
# Save the current directory (should be qhana-plugin-runner/stable_plugins/state_analysis/test/unittest)
ORIG_DIR=$(pwd)
...
poetry run pytest -v --tb=long --cov=common.state_cropper -vv --cov=common.plugin_utils.marshmallow_util --cov-report=term-missing  test_state_cropper.py marshmallow_tests.py 
