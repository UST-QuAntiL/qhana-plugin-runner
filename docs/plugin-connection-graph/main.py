from plugin_extractor import get_plugin_information
from dash import Dash, html, dcc, Input, Output, callback, ctx
import dash_cytoscape as cyto
import re

all_plugins_md_file_path = "../all-plugins.md"
# all_plugins_md_file_path = "example-all-plugins.md"

"""
Requires pandas, dash and dash-cytoscape installed.

The plugin description file contains information about every plugins input and output.
This script uses plugin_extractor.py to get the information into arrays and dictionaries,
so that it then can be visualized using dash.
...
def create_graph(plugins):
    ...
    for data_type in data_types:
        elements.append({'data': {'id': data_type, 'label': break_word(data_type)}, 'classes': 'data-types'})
        for (plugin_name, plugin) in plugins.items(): # example plugin = {'input': [['executable/circuit', ['text/x-qasm'], True]], 'output': [['circuit/*', ['text/html'], True]]}
            inputs = []
            ...
            if inputs.__contains__(data_type):
                elements.append({'data': {'id': plugin_name + ' input - ' + data_type, 'label': break_word(plugin_name.split(" (@")[0])}})
            if outputs.__contains__(data_type):
                elements.append({'data': {'id': plugin_name + ' output - ' + data_type, 'label': break_word(plugin_name.split(" (@")[0])}})
        ...
        for (plugin_name, plugin) in plugins.items():
            for input in plugin["input"]:
                if data_type == input[0]:
                    elements.append({'data': {'source': plugin_name + ' input - ' + data_type, 'target': input[0]}, 'classes': 'input-edge'})

            for output in plugin["output"]:
                if data_type == output[0]:
                    elements.append({'data': {'source': output[0], 'target': plugin_name + ' output - ' + data_type}, 'classes': 'output-edge'})
...
def show_graph(data):
    ...
    app.layout = html.Div([
        ...
        html.Div(
            children=[
                html.H4("Legend"),
                html.Div(children=[
                    html.Span(style={'background-color': 'orange', 'padding': '0px 5px', 'margin-right': '5px'}),
                    "Used as Input"
                ]),
                html.Div(children=[
                    html.Span(style={'background-color': 'cyan', 'padding': '0px 5px', 'margin-right': '5px'}),
                    "Output produced"
                ])
            ],
            style={
    ...
    @callback(
        Output("plugin-graph", "generateImage"),
        Input("svg", "n_clicks")
    )
...
plugins = get_plugin_information(all_plugins_md_file_path)
