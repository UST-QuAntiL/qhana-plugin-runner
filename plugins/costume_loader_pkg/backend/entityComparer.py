...
from plugins.costume_loader_pkg.backend.elementComparer import (
    ElementComparerType,
    ElementComparerFactory,
)
from plugins.costume_loader_pkg.backend.attributeComparer import (
    AttributeComparerType,
    AttributeComparerFactory,
)
from plugins.costume_loader_pkg.backend.transformer import (
    TransformerType,
    TransformerFactory,
)
from plugins.costume_loader_pkg.backend.aggregator import (
    AggregatorType,
    AggregatorFactory,
)
import enum
...
class EmptyAttributeAction(enum.Enum):
    ...
    def get_name(variable) -> str:
        if variable == EmptyAttributeAction.ignore:
            ...
            logging.error(
                'No name for EmptyAttributeAction "' + str(variable) + '" specified'
            )
            raise ValueError(
                'No name for EmptyAttributeAction "' + str(variable) + '" specified'
            )
...
class EntityComparer:
    ...
    def __add_similarity_to_cache(
        self, first: [Any], second: [Any], similarity: float
    ) -> None:
    ...
    def add_element_comparer(
        self, attribute: Attribute, elementComparerType: ElementComparerType
    ) -> None:
    ...
    def add_attribute_comparer(
        ...
        attributeComparer = AttributeComparerFactory.create(
            attributeComparerType, elementComparer, attribute
        )
        ...
        for attribute in first.attributes:
            ...
            if attribute not in second.attributes:
                ...
                if emptyAttributeAction == EmptyAttributeAction.ignore:
                    ...
                    logging.warning(
                        "Unknown value for EmptyAttributeAction. This attribute will be ignored."
                    )
                    continue
            ...
            if self.__is_similarity_in_cache(firstAttribute, secondAttribute):
                ...
                try:
                    temp = attributeComparer.compare(firstAttribute, secondAttribute)
                    ...
                    if emptyAttributeAction == EmptyAttributeAction.ignore:
                        ...
                        errMessage = (
                            errMessage + " The attribute will be evaluated as zero."
                        )
                        logging.warning(errMessage)
                        aggregationValues.append(0.0)
                        continue
    ...
    def calculate_distance(self, first: Entity, second: Entity) -> float:
        return self.similarityTransformer.transform(
            self.calculate_similarity(first, second)
        )
...
class CostumeComparer(EntityComparer):
    ...
    def __init__(
        ...
        # dominanteFarbe
        self.add_element_comparer(Attribute.dominanteFarbe, ElementComparerType.wuPalmer)
        # dominanteCharactereigenschaft
        self.add_element_comparer(
            Attribute.dominanteCharaktereigenschaft, ElementComparerType.wuPalmer
        )
        # dominanterZustand
        self.add_element_comparer(
            Attribute.dominanterZustand, ElementComparerType.wuPalmer
        )
        # stereotyp
        self.add_element_comparer(
            Attribute.stereotyp,
