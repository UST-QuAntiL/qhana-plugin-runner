...
class AggregatorType(enum.Enum):
    ...
    def get_name(aggregatorType) -> str:
        ...
        if aggregatorType == AggregatorType.mean:
            ...
            logging.error(
                'No name for aggregator "' + str(aggregatorType) + '" specified'
            )
            raise ValueError(
                'No name for aggregator "' + str(aggregatorType) + '" specified'
            )
    ...
    def get_description(aggregatorType) -> str:
        ...
        if aggregatorType == AggregatorType.mean:
            description += (
                "Aggregates an array of floating point values " + "with using the mean."
            )
        elif aggregatorType == AggregatorType.median:
            description += (
                "Aggregates an array of floating point values " + "with using the median."
            )
        elif aggregatorType == AggregatorType.max:
            description += (
                "Aggregates an array of floating point values "
            ...
            logging.error(
                'No description for aggregator "' + str(aggregatorType) + '" specified'
            )
            raise ValueError(
                'No description for aggregator "' + str(aggregatorType) + '" specified'
            )
