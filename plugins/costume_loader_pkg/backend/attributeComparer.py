...
from plugins.costume_loader_pkg.backend.elementComparer import ElementComparer
...
class AttributeComparerType(enum.Enum):
    ...
    def get_name(attributeComparerType) -> str:
        ...
        if attributeComparerType == AttributeComparerType.symMaxMean:
            ...
            logging.error(
                'No name for attribute comparer "'
                + str(attributeComparerType)
                + '" specified'
            )
            raise ValueError(
                'No name for attribute comparer "'
    ...
    def get_description(attributeComparerType) -> str:
        ...
        if attributeComparerType == AttributeComparerType.symMaxMean:
            description += (
                "Compares two attributes, i.e. sets of elements accordingly to "
            ...
            logging.error(
                'No description for attribute comparer "'
                + str(attributeComparerType)
                + '" specified'
            )
            raise ValueError(
...
class AttributeComparer(metaclass=ABCMeta):
    ...
    def add_similarity_to_cache(
        self, firstElement: Any, secondElement: Any, similarity: float
    ) -> None:
