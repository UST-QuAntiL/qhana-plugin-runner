...
from plugins import costume_loader_pkg
from plugins.costume_loader_pkg.backend.entity import Entity, EntityFactory
from plugins.costume_loader_pkg.backend.attribute import Attribute
from plugins.costume_loader_pkg.backend.attributeComparer import (
    AttributeComparerType,
)
...
from plugins.costume_loader_pkg.backend.entityComparer import (
    EntityComparer,
    EmptyAttributeAction,
)
import os
import shutil
import enum

Subset5PositiveFileName = (
    os.path.dirname(costume_loader_pkg.__file__) + "/subsets/5/PositiveSubset5.csv"
)
Subset5NegativeFileName = (
    os.path.dirname(costume_loader_pkg.__file__) + "/subsets/5/NegativeSubset5.csv"
)
Subset10PositiveFileName = (
    os.path.dirname(costume_loader_pkg.__file__) + "/subsets/10/PositiveSubset10.csv"
)
Subset10NegativeFileName = (
    os.path.dirname(costume_loader_pkg.__file__) + "/subsets/10/NegativeSubset10.csv"
)
Subset25PositiveFileName = (
    os.path.dirname(costume_loader_pkg.__file__) + "/subsets/25/PositiveSubset25.csv"
)
Subset25NegativeFileName = (
    os.path.dirname(costume_loader_pkg.__file__) + "/subsets/25/NegativeSubset25.csv"
)
Subset40PositiveFileName = (
    os.path.dirname(costume_loader_pkg.__file__) + "/subsets/40/PositiveSubset40.csv"
)
Subset40NegativeFileName = (
    os.path.dirname(costume_loader_pkg.__file__) + "/subsets/40/NegativeSubset40.csv"
)
...
class Subset(enum.Enum):
    ...
    def get_name(subset) -> str:
        if subset == Subset.subset5:
            ...
            logging.error('No name for subset "' + str(subset) + '" specified')
            raise ValueError('No name for subset "' + str(subset) + '" specified')

    @staticmethod
    def get_subset(subsetString) -> "Subset":
        if subsetString == "Subset5":
            ...
            logging.error('No subset for name "' + str(subsetString) + '" specified')
...
class EntityService:
    ...
    def get_costume_with(self, costumes, kostuemId, rollenId, filmId):
        for elem in costumes:
            if (
                str(elem.kostuemId) == kostuemId
                and str(elem.rollenId) == rollenId
                and str(elem.filmId == filmId)
            ):
                return elem
        return None
    ...
    def create_components(self) -> None:
        self.entitiyComparer = EntityComparer(self.aggregatorType, self.transformerType)

        for attribute in self.elementComparerType.keys():
            self.entitiyComparer.add_element_comparer(
                attribute, self.elementComparerType[attribute]
            )
    ...
    def create_caches(self) -> None:
        logging.debug("Start creating caches")
        for attribute in self.entitiyComparer.elementComparer.keys():
            logging.debug("Start caching " + Attribute.get_name(attribute))
