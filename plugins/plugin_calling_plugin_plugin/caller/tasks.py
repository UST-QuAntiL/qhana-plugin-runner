...
@CELERY.task(name=f"{Caller.instance.identifier}.processing_task_1", bind=True)
def processing_task_1(self, db_id: int) -> str:
    ...
    if invoked_plugin is None:
        raise ValueError("No invoked plugin provided!")

    out_str += "\nInvoked plugin: " + invoked_plugin
...
@CELERY.task(name=f"{Caller.instance.identifier}.processing_task_2", bind=True)
