...
class QiskitQkNN(QkNN):
    ...
    def __init__(self, train_data, train_labels, k, backend, shots):
        super(QiskitQkNN).__init__(train_data, train_labels, k, backend, shots)
    ...
    def fit(self):
        ...
        if self.train_data.shape[1] < n_variables:
            self.train_data = np.pad(self.train_data, [(0, 0), (0, n_variables - self.train_data.shape[1])], mode='constant',
                                constant_values=0)

        encoded_train_data = analog.encode(self.train_data)

        self.qknn.fit(encoded_train_data, self.train_labels)
