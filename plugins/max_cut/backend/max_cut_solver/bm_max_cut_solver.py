...
class BmMaxCutSolver(MaxCutSolver):
    """
    Instantiates the BmMaxCutSolver with the graph.
    """
    ...
    """
    Solves the max cut problem using a Burer-Monteiro
    approach and returns the
    ...
    def solve(self):
        ...
        TASK_LOGGER.debug("Start solving maxcut using BM solver")
        TASK_LOGGER.debug("Problem size (i.e. edges) = " + str(probsize))

        sdp = MaxCutBM(self.graph)
        ...
        return (largestCutValue, self.__calculate_cut_edges(nodes, largestCutSubset))
