...
from plugins.workflows.util.helper import request_json, get_form_variables
...
def human_task_watcher(self, db_id: int, camunda_config: dict) -> None:
    ...
    for human_task in human_tasks:
        ...
        if (human_task.delegation_state == "PENDING" or human_task.delegation_state is None) and \
            ...
            # TODO: Remove
            href = f"http://localhost:5005/plugins/workflows@v0-3-0/{db_id}/demo-step-process/"
            ui_href = f"http://localhost:5005/plugins/workflows@v0-3-0/{db_id}/demo-step-ui/"

            # Add new sub-step task for input gathering
            new_sub_step_task = add_step.s(
                db_id=db_id, step_id=human_task.id, href=href, ui_href=ui_href, prog_value=42,
                task_log="",
            )
