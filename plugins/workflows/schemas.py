    ...
    def __init__(self, params=None):
        ...
        for key, val in params.items():
            ...
            if not val["value"]:
                ...
                continue
            if val["type"] == "String" and val["value"].startswith(
                f"{prefix_choices}{prefix_delimiter}"
            ):
                choices = (
                    val["value"][
                        len(prefix_choices) + len(prefix_delimiter) : len(val["value"])
                    ]
                ).split(",")
                choices_dict = {}
                ...
                self.__setattr__(key, inputs[key])
            elif val["type"] == "String" and val["value"].startswith(
                f"{prefix_enum}{prefix_delimiter}"
            ):
                choices = (
                    val["value"][
                        len(prefix_enum) + len(prefix_delimiter) : len(val["value"])
                    ]
                ).split(",")
                choices_dict = {}
                for choice in choices:
                ...
                f"{prefix_file_url}{prefix_delimiter}"
            ):
                data_input_type, data_content_types = (
                    val["value"][
                        len(prefix_file_url) + len(prefix_delimiter) : len(val["value"])
                    ]
                ).split(",")
                inputs[key] = FileUrl(
                    required=True,
