...
class BaseQuantumKMeans:
    ...
    def ExecuteNegativeRotation(
        ...
        if plotData:
            self.PlotRawData(
                [dataRaw, centroidsRaw],
                ["Raw data", "Raw centroids"],
                "Initial raw data",
                "InitialRaw",
                whichCircuit,
            )
        ...
        if plotData:
            self.PlotData(
                [data, centroids],
                ["Preprocessed data", "Preprocessed centroids"],
                "Preprocessed data",
                "InitialPreprocessed",
                whichCircuit,
            )
        ...
        while notConverged and counter < maxRuns:
            ...
            notConverged = not self.CheckConvergency(
                oldCentroidMapping, newCentroidMapping, relativeResidualNumber
            )
            if notConverged:
                centroids = self.CalculateCentroids(
                    newCentroidMapping, centroids, data, k
                )
    ...
    def CheckConvergency(
        self, oldCentroidMapping, newCentroidMapping, relativeResidualNumber
    ):
        """
        ...
        self.UpdateConsole(
            residual=round(countOfDifferentLabels / amountOfDataPoints * 100, 2)
        )
    ...
    def CalculateAngles(self, cartesianPoints):
        ...
        for i in range(0, len(angles)):
            ...
            # here: |a| = |b| = 1
            angles[i] = acos(
                self.baseVector[0] * cartesianPoints[i][0]
                + self.baseVector[1] * cartesianPoints[i][1]
            )
        return angles
    ...
    def ApplyNegativeRotationCircuit(
        ...
        while queueNotEmpty:
            ...
            qubitsForCircuit = (
                maxQubits if maxQubits < maxQubitsForCircuit else maxQubitsForCircuit
            )
            ...
            def circ_func():
                ...
                for i in range(0, qubitsForCircuit):
                    ...
                    qml.RY(
                        -parameters[index][1], wires=i
                    )  # negative centeroidAngle rotation
                    index += 1
            ...
            circuitsIndex += 1
            self.UpdateConsole(
                circuitAmount=ceil(globalWorkAmount / maxQubits),
                currentCircuit=ceil(index / maxQubits),
            )
    ...
    def ApplyDestructiveInterferenceCircuit(
        ...
        while queueNotEmpty:
            ...
            circuitsIndex += 1
            self.UpdateConsole(
                circuitAmount=ceil(globalWorkAmount * 2 / maxQubits),
                currentCircuit=ceil(index * 2 / maxQubits),
            )
    ...
    def MapHistogramToQubitHits(self, histogram):
        ...
        for basisState in histogram:
            for i in range(0, length):
                ...
                if basisState[length - i - 1] == "0":
                    qubitHits[i] = (
                        qubitHits[i][0] + histogram[basisState],
                        qubitHits[i][1],
                    )
                else:
                    qubitHits[i] = (
                        qubitHits[i][0],
                        qubitHits[i][1] + histogram[basisState],
                    )
    ...
    def UpdateConsole(
        self, run=None, circuitAmount=None, currentCircuit=None, residual=None
    ):
        if run is None:
            run = self.runSafe
        else:
        ...
        printText += (
            "Circuit "
            + str(self.currentCircuitSafe)
            + " / "
            + str(self.circuitAmountSafe)
            + "     "
        )
...
class NegativeRotationQuantumKMeans(BaseQuantumKMeans):
    ...
    def Run(self, data, plotData=False, plotCircuit=False) -> Tuple[np.ndarray, str]:
        ...
        clusterMapping, representative_circuit = self.ExecuteNegativeRotation(
            ...
            self.shotsEach,
            self.maxRuns,
            self.relativeResidualAmount / 100.0,
            self.backend,
            plotData,
            plotCircuit,
            "negrot",
...
class PositiveCorrelationQuantumKmeans:
    def fit(self, data, k, max_iter, tol, backend: qml.Device, shots_each):
        ...
        centroids = BaseQuantumKMeans.Normalize(
            BaseQuantumKMeans.Standardize(self.generate_centroids(k, 2))
        )
        ...
        while not converged and it < max_iter:
            ...
            it += 1
            new_centroid_mapping = self.execute_circuits(
                data_phi, data_theta, centroids_phi, centroids_theta, backend, shots_each
            )
            converged = self.check_convergence(
                old_centroid_mapping, new_centroid_mapping, tol / 100
            )
            old_centroid_mapping = new_centroid_mapping
            centroids = BaseQuantumKMeans.CalculateCentroids(
                new_centroid_mapping, centroids, data, k
            )
            new_centroids = np.zeros((k, 2))
            for i in range(0, k):
                for j in range(0, 2):
