...
class StatePreparationQuantumKMeans(Clustering):
    ...
    def quantum_circuit(
        ...
        qml.RY(data_angle, wires=measure_wire)  # angle for data point
        qml.RY(centroid_angle, wires=ancilla_wire)  # angle for centroid
        qml.CNOT(wires=[measure_wire, ancilla_wire])
        qml.Hadamard(measure_wire)
    ...
    def simpler_plot(self, prepped_data, prepped_centroids, centroid_mapping):
        ...
        for i in range(len(centroid_mapping)):
            clusters[centroid_mapping[i]].append(list(prepped_data[i]))
