...
from plugins.optimization_coordinator import OPTI_COORD_BLP, OptimizationCoordinator
from plugins.optimization_coordinator.schemas import (
    InputDataSchema,
    TaskResponseSchema,
)
from plugins.optimization_coordinator.tasks import preprocessing_task, processing_task
...
@OPTI_COORD_BLP.route("/ui/")
class MicroFrontend(MethodView):
    """Micro frontend for the optimization coordinator plugin."""
    ...
    @OPTI_COORD_BLP.html_response(
        HTTPStatus.OK,
        description="Micro frontend of the optimization coordinator plugin.",
    )
    @OPTI_COORD_BLP.arguments(
        InputDataSchema(partial=True, unknown=EXCLUDE, validate_errors_as_result=True),
        location="query",
        required=False,
    )
    @OPTI_COORD_BLP.require_jwt("jwt", optional=True)
    ...
    def render(self, data: Mapping, errors: dict):
        ...
        return Response(
            render_template(
                ...
                values=data,
                errors=errors,
                process=url_for(f"{OPTI_COORD_BLP.name}.ProcessView"),
                example_values=url_for(
                    f"{OPTI_COORD_BLP.name}.MicroFrontend", **self.example_inputs
                ),
            )
        )
...
@OPTI_COORD_BLP.route("/process/")
class ProcessView(MethodView):
    """Start a long running processing task."""

    @OPTI_COORD_BLP.arguments(InputDataSchema(unknown=EXCLUDE), location="form")
    ...
    def post(self, arguments):
        ...
        step_id = "demo-step"
        href = url_for(
            f"{OPTI_COORD_BLP.name}.DemoStepView", db_id=db_task.id, _external=True
        )
        ui_href = url_for(
            f"{OPTI_COORD_BLP.name}.DemoStepFrontend", db_id=db_task.id, _external=True
        )
...
@OPTI_COORD_BLP.route("/<int:db_id>/demo-step-ui/")
    ...
    def render(self, data: Mapping, db_id: int, errors: dict):
        ...
        return Response(
            render_template(
                ...
                values=data,
                errors=errors,
                process=url_for(f"{OPTI_COORD_BLP.name}.DemoStepView", db_id=db_id),
                example_values=url_for(
                    f"{OPTI_COORD_BLP.name}.DemoStepFrontend",
                    db_id=db_id,
                    **self.example_inputs,
                ),
            )
...
@OPTI_COORD_BLP.route("/<int:db_id>/demo-step-process/")
