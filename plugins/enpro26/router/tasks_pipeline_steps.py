...
def run_pipeline_step(celery_task, db_id: int, step_log_name: str, step_action: callable):
    ...
    try:
        ...
        task_data.add_task_log_entry(f"CRASH in {step_log_name} step:\n{traceback.format_exc()}")
        save_task_error.delay(failing_task_id=celery_task.request.id, db_id=db_id)

def invoke_plugin_and_subscribe(task_data: ProcessingTask, plugin_name: str, payload: dict, step_name: str, url_key: str):
...
def start_wu_palmer(self, db_id: int):
    ...
    def action(task_data):
        ...
        invoke_plugin_and_subscribe(task_data, "wu_palmer", payload, "Wu-Palmer", "wu_palmer_url")
...
def start_mapping(self, db_id: int):
    ...
    def action(task_data):
        ...
        invoke_plugin_and_subscribe(task_data, "mapping", payload, "Mapping Distances", "mapping_url")
    ...
    def action(task_data):
        ...
        if params.include_intermediate_results_in_output:
            STORE.persist_task_result(
                db_id, open_url(element_sims_url).content, "wu_palmer_similarities.zip",
        ...
        invoke_plugin_and_subscribe(task_data, "transformers", payload, "Transformers", "transformers_url")
...
def start_aggregator(self, db_id: int, source_url: str):
    ...
    def action(task_data):
        ...
        if params.include_intermediate_results_in_output:
            ...
            STORE.persist_task_result(
                db_id, open_url(element_dists_url).content, f"{prefix}_element_distances.zip",
        ...
        invoke_plugin_and_subscribe(task_data, "aggregator", payload, "Aggregator", "aggregators_url")
...
def start_mds(self, db_id: int, source_url: str):
    ...
    def action(task_data):
        ...
        if params.include_intermediate_results_in_output:
            prefix = task_data.data.get("current_pipeline", "unknown")
            STORE.persist_task_result(
                db_id, open_url(attr_dists_url).content, f"{prefix}_attribute_distances.zip",
...
def finalize_pipeline(self, db_id: int, source_url: str):
    ...
    def action(task_data):
        ...
        prefix = task_data.data.get("current_pipeline", "unknown")
        STORE.persist_task_result(
            db_id, open_url(final_dists_url).content, f"{prefix}_mds_final_vectors.zip",
