...
def launch_next_pipeline(task_data: ProcessingTask):
    ...
    if next_pipeline == "wu_palmer":
        ...
        task_data.add_task_log_entry(
            "Starting Distance Mapping Pipeline. Includes: Distance Mapping, Aggregator, MDS"
        )
        task_data.save(commit=True)
...
def start_wu_palmer(self, db_id: int):
    ...
    try:
        ...
        task_data.add_task_log_entry(
            f"Error occured during wu-palmer plugin. Attempting to retry:\n{e}"
        )
        task_data.save(commit=True)
        raise self.retry(exc=e, countdown=CELERY_COUNTDOWN)
    except Exception as e:
        task_data.add_task_log_entry(
            f"CRASH in wu_palmer step:\n{traceback.format_exc()}"
        )
...
def start_mapping(self, db_id: int):
    ...
    try:
        ...
        task_data.add_task_log_entry(
            f"Error occured during mapping plugin. Attempting to retry:\n{e}"
        )
        task_data.save(commit=True)
        raise self.retry(exc=e, countdown=CELERY_COUNTDOWN)
    except Exception as e:
...
def start_transformers(self, db_id: int, source_url: str):
    ...
    try:
        ...
        task_data.add_task_log_entry(
            f"Error occured during transformers plugin. Attempting to retry:\n{e}"
        )
        task_data.save(commit=True)
        raise self.retry(exc=e, countdown=CELERY_COUNTDOWN)
    except Exception as e:
        task_data.add_task_log_entry(
            f"CRASH in transformers step:\n{traceback.format_exc()}"
        )
        save_task_error.delay(failing_task_id=self.request.id, db_id=db_id)
...
def start_aggregator(self, db_id: int, source_url: str):
    ...
    try:
        ...
        task_data.add_task_log_entry(
            f"Error occured during aggregator step. Attempting to retry:\n{e}"
        )
        task_data.save(commit=True)
        raise self.retry(exc=e, countdown=CELERY_COUNTDOWN)
    except Exception as e:
        task_data.add_task_log_entry(
            f"CRASH in aggreagtor step:\n{traceback.format_exc()}"
        )
        save_task_error.delay(failing_task_id=self.request.id, db_id=db_id)
...
def start_mds(self, db_id: int, source_url: str):
    ...
    try:
        ...
        task_data.add_task_log_entry(
            f"Error occured during mds step. Attempting to retry:\n{e}"
        )
        task_data.save(commit=True)
        raise self.retry(exc=e, countdown=CELERY_COUNTDOWN)
    except Exception as e:
...
def finalize_pipeline(self, db_id: int, source_url: str):
    ...
    try:
        ...
        task_data.add_task_log_entry(
            f"Error occured during finalization step. Attempting to retry:\n{e}"
        )
        task_data.save(commit=True)
        raise self.retry(exc=e, countdown=CELERY_COUNTDOWN)
    except Exception as e:
