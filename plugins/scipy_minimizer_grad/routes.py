...
@SCIPY_MINIMIZER_GRAD_BLP.route("/")
class MetadataView(MethodView):
    """Plugins collection resource."""

    @SCIPY_MINIMIZER_GRAD_BLP.response(HTTPStatus.OK, PluginMetadataSchema)
    ...
    def get(self):
        ...
        return PluginMetadata(
            ...
            tags=ScipyMinimizerGrad.instance.tags,
...
@SCIPY_MINIMIZER_GRAD_BLP.route("/ui-setup/")
...
@SCIPY_MINIMIZER_GRAD_BLP.route("/process-setup/")
class MinimizerSetupProcessStep(MethodView):
    ...
    def post(self, arguments: MinimizerSetupTaskInputData, callback: CallbackUrl):
        ...
        db_task.add_next_step(
            href=url_for(
                f"{SCIPY_MINIMIZER_GRAD_BLP.name}.{MinimizationEndpoint.__name__}",
                db_id=db_task.id,
                _external=True,
            ),
            ui_href=url_for(
                f"{SCIPY_MINIMIZER_GRAD_BLP.name}.{MinimizationMicroFrontend.__name__}",
                db_id=db_task.id,
                _external=True,
            ),
            step_id="minimize",
...
@SCIPY_MINIMIZER_GRAD_BLP.route("/task/<int:db_id>/ui-minimize/")
class MinimizationMicroFrontend(MethodView):
    ...
    def render(self, data: Mapping, errors: dict, db_id: int):
        plugin = ScipyMinimizerGrad.instance
        if plugin is None:
...
@SCIPY_MINIMIZER_GRAD_BLP.route("/task/<int:db_id>/minimize/")
class MinimizationEndpoint(MethodView):
    """Endpoint for the minimization."""

    @SCIPY_MINIMIZER_GRAD_BLP.arguments(MinimizeSchema(unknown=EXCLUDE), location="form")
